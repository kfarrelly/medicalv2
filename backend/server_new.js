...
app.post('/sendResetMail', (req, res) => {
	...
	User.findOne({ 'email': req.body.email }).then((result) => {
			...
			var update = {'activationKey': actkey};
					...
					User.findOneAndUpdate({_id:result._id.toString()}, update, options).then(( doc) =>{
						var mailOptions = {
...
//Delete pkg transport
	...
	app.get('/profile', (req, res) => {
		User.findOne({ 'token': req.query.token }).then((result) => {
			res.status(200).send(result);
		}).catch((err) => {
			res.status(400).send(err);
		});
	...
	app.post('/updateprofile/:id', (req, res) => {
		...
		var options = { new: false };
		User.findOneAndUpdate({ publicKey:req.params.id}, update, options).then(( doc) =>{
			res.status(200).send(doc);
			console.log('responser',doc);
		}).catch((err) => {
			res.status(400).send(err);
